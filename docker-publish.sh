[ -z "$1" ] && { echo "Missing 1st argument: BalkanID github commit SHA"; exit 1; }
...
[ -z "$6" ] && { echo "Missing 6th argument: BALKANID version"; exit 1; }

BALKANID_GITHUB_COMMIT="$1"
DOCKER_REPO="$2"
DOCKER_USER="$3"
DOCKER_PASS="$4"
SCRIPT_DIR="$5"
BALKANID_VERSION="v$6"
IS_PRERELEASE="$7"
...
sudo docker buildx build --build-arg BALKANID_GITHUB_COMMIT=$BALKANID_GITHUB_COMMIT \
                         -t $DOCKER_REPO/postgrest:$BALKANID_VERSION-arm \
                         --push .
...
sudo docker buildx imagetools create --append -t $DOCKER_REPO/BALKANID:$BALKANID_VERSION $DOCKER_REPO/postgrest:$BALKANID_VERSION-arm
[ -z $IS_PRERELEASE ] && sudo docker buildx imagetools create --append -t $DOCKER_REPO/postgrest:latest $DOCKER_REPO/postgrest:$BALKANID_VERSION-arm
